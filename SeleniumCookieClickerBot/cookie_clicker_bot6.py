#   Below is my sixth attempt at a cookie clicker algorithm. Seeing how many cookies per second we can reach in 5 mins!
#   This attempt buys the most expensive available upgrade after each interval... the interval starts at (interval)
#   seconds and increases by (interval_delta) with each cycle! Let's see if it makes an impact!
#   My C/S score for this attempt was 90. Not bad! Maybe a little fast on the interval ramp up.
...
timeout = time.time() + interval      # TEN SECOND TIMER
...
while keep_going:
    ...
    if time.time() > timeout:
        ...
        try:
            ...
            timeout = time.time() + interval   # basically resets the 10-second cycle timer
